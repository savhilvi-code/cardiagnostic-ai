...
(async()=>{
  ...
  await context.route('**/*',async route=>{
    ...
    if(p==='/api/quota')return json({quota:{remaining:3,limit:5,unlimited:false}});
  ...
  await page.waitForFunction(()=>document.querySelector('#systemPill').textContent.includes('3 of 5'));
  ...
  console.log('PASS: startup, auth/logout, empty/manual/VIN failure/save failure/create, multiple vehicles and isolation, tabs/specs/timeline, structured problem + chat context, F5 restore, 12h expiry, chat viewport, trash/restore, mobile navigation/overflow. No live writes.');
